...
if [ "$(ls -l /backups/*.dump.tar.gz | grep -v ^l | wc -l)" -gt "${BACKUP_ROTATIONS}" ]; then
  echo "[Backup] ${date} *** Removing deprecated dumps..."
  ls -F /backups/*.dump.tar.gz | head -n -${BACKUP_ROTATIONS} | xargs rm
  echo "[Backup] ${date} *** Done"
fi
